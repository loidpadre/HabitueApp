...
export const Card = ({ casa }) => {
    ...
    <img src={image.imagePrincipal} className="w-full h-60 object-cover rounded-t-lg" alt="Imagem da Casa" />
    <div className="p-5 text-gray-800">
        <div className="flex justify-between items-center">
            <h1 className="text-2xl font-semibold text-orange-600">R$ {price}</h1>
            <p className="text-md bg-orange-700 text-white p-2 rounded-md"> {localization}</p>
        </div>
        <div className="py-2">
            <p className="text-sm font-semibold">Rua: <span className="font-normal">{rua}</span></p>
        </div>
        <div className="flex flex-col justify-center items-end">
            <button className="mt-4 p-2 rounded-md w-full bg-zinc-900 text-white hover:bg-zinc-800 transition duration-300">
                <Link to={`detail/${id}`} className="w-full h-full flex justify-center items-center">Ver detalhes</Link>
            </button>
